  ...
  return (
    <section className=" h-[500dvh] bg-background" ref={targetref}>
      <div className=" sticky top-0 h-screen flex bg-muted-foreground items-center overflow-hidden min-w-full ">
        <motion.div className=" flex gap-4 min-w-full " style={{
            x
        }}>
         {
            Array(50).fill(0).map((_,i)=><div>
                <Image src={`https://picsum.photos/200/300?random=${i}`} width={500} height={500} alt={"random image"} className=" min-h-[500px] flex flex-1 w-[50dvw] min-w-[300px]" />
            </div>
            )
         }
        </motion.div>
